...
### 1.2.6 Gating Strategy Visualization

The following figures provide a comprehensive overview of the sequential gating strategy applied to each cell line.
...
if (!is.null(gate_plots) && length(gate_plots) > 0) {
  for (line in names(gate_plots)) {
    ...
    # Render the combined grob
    grid::grid.draw(gate_plots[[line]])
    
    cat(sprintf('<p class="figure-caption">Figure: Sequential gating strategy for %s. Stages (from left to right): %s morphological classification (FSC/SSC and Singlets), and functional staining (Annexin V / PI).</p>\n', 
  ...
  cat('<p><em>Comprehensive gating visualization not available.</em></p>\n')
}
```

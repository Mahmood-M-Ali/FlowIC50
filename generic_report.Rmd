...
### 1.3.4 Gate 1: Cell Population (FSC-A vs SSC-A)
...
### 1.3.5 Gate 2: Singlet Discrimination (FSC-A vs FSC-H)
...
if (!is.null(singlet_plots) && length(singlet_plots) > 0) {
  ...
  cat('<p><em>Singlet gate visualization not available</em></p>\n')
}
```

### 1.3.6 Gate 3: Multi-Channel Marker Strategy

```{r results='asis', fig.height=5, fig.width=7}
marker_plots <- params$marker_plots

if (!is.null(marker_plots) && length(marker_plots) > 0) {
  for (key in names(marker_plots)) {
    # Key is 'CellLine_Channel'
    cat('<div class="figure">\n')
    cat(sprintf('<h4>Marker Gate: %s</h4>\n', key))
    print(marker_plots[[key]])
    cat('<p class="figure-caption">Figure: Fluorescence histogram showing the distribution of marker intensity and the applied positivity threshold (dashed line).</p>\n')
    cat('</div>\n')
    cat('<hr style="margin: 30px 0;">\n')
  }
} else {
  cat('<p><em>Marker gate visualization not available</em></p>\n')
}
```

### 1.3.7 Gating Strategy Overview (Combined)
